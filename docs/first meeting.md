...
1. 앱에서 내장 DB사용 : Push 정보, 자동로그인
2. 모든 소분류 탭에서 뒤로가기는 메인화면 (메인에서 뒤로가기는 앱 종료)
3. Push는 개인 사용자가 지정한 시간에는 소리나 진동으로 알리지 않고, 그냥 Push만 뜨도록 함.
...
2. UI / App-Icon 구성
3. 리스트의 한 페이지 글 개수
4. 모든 글에서의 댓글 여부 (+댓글 달리면 원글 작성자에게 Push 전송?)
5. 팀플실 : 과사와 논의 필요
6. 모든 공지사항, 소분류, 대분류 별 새 글 알림 (ex. N 마크 달기) 여부 : Push가 없으면 쉽게 접하기 힘드므로
7. Push 알림을 좀 더 다양한 데 적용 : 취업게시판을 꾸준히 확인하고 싶은 사람이 있을 수 있으니깐..? 
8. 개발자에게 문의하기 기능 -> 학생회 오픈채팅방처럼 학생